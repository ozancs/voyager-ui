...
function addField() {
  ...
  if (!state.settings.quickConfig.some((x) => x.section === a.section && x.key === a.key))
    state.settings.quickConfig.push({ section: a.section, key: a.key, unit: a.unit || '' });
  saveSettings();
  adding.value = null;
}
</script>
<template>
  <div class="page">
    <div class="head" :class="{ dirty: changes.length }">
      <span class="d"></span>
      <b>{{
        changes.length ? tn(changes.length, '{n} unsaved change', '{n} unsaved changes') : t('Printer settings')
      }}</b>
      <span class="mu">{{
        t('Edits the matching line in your .cfg files (SAVE_CONFIG block first). Klipper needs a restart to apply.')
      }}</span>
      <div class="grow"></div>
      <button class="btn" @click="adding = { section: '', key: '', unit: '' }">
        <Icon name="plus" :size="16" />{{ t('Add field') }}
      </button>
      <button class="btn" :disabled="!changes.length" @click="drafts = {}">
        <Icon name="x" :size="16" />{{ t('Discard') }}
      </button>
      <button class="btn" :disabled="!changes.length || saving" @click="save(false)">
        <Icon name="save" :size="16" />{{ t('Save') }}
      </button>
      <button class="btn acc lg" :disabled="!changes.length || saving || isPrinting" @click="save(true)">
        <Icon name="restart" :stroke="2.4" />{{ saving ? t('Saving…') : t('Save & Restart') }}
      </button>
    </div>
    <div class="grid3">
      <section v-for="(fields, sec) in groups" :key="sec" class="card">
        <div class="card-h">
          <h2 class="mono" style="font-size: 14px">[{{ sec }}]</h2>
        </div>
        <div class="g2">
          <label v-for="f in fields" :key="f.key" class="fld">
            <span class="row" style="justify-content: space-between"
              ...
              ><button
                class="btn clear ibtn sm rm"
                :aria-label="t('Remove {k}', { k: f.key })"
                @click.prevent="removeField(f)"
              >
            ...
            <span class="ib" :class="{ ch: drafts[id(f)] !== undefined && drafts[id(f)] !== current(f) }">
              <input
                ...
                @input="drafts[id(f)] = $event.target.value"
                :aria-label="f.key"
                spellcheck="false"
              />
              <span v-if="f.unit" class="u">{{ f.unit }}</span>
            </span>
            <span v-if="drafts[id(f)] !== undefined && drafts[id(f)] !== current(f)" class="was mono">{{
              t('was {v}', { v: current(f) })
            }}</span>
          </label>
        </div>
    ...
    <div v-if="!Object.keys(groups).length" class="empty">
      {{ t('No fields match your config yet. Use “Add field”.') }}
    </div>
  </div>
  <Modal v-if="result" :title="t('Saved')" width="620px" @close="result = null">
    <div
      ...
      :key="i"
      class="row mono"
      style="font-size: 12px; gap: 12px; padding: 6px 0; border-bottom: 1px solid var(--bd)"
    >
      <Icon
        :name="l.error ? 'warn' : 'check'"
        :size="16"
        :style="{ color: l.error ? 'var(--dg)' : 'var(--ok)', flexShrink: 0 }"
      />
      <span class="grow">[{{ l.f.section }}] {{ l.f.key }} = {{ l.v }}</span>
      <span class="mu">{{ l.error ? t(l.error) : l.file + ' · ' + l.where }}</span>
    </div>
    <template #foot
      ><button class="btn lg" @click="result = null">{{ t('Close') }}</button
      ><button
        class="btn lg acc"
        :disabled="isPrinting"
        @click="
  ...
  <Modal v-if="adding" :title="t('Add field')" @close="adding = null">
    <label class="col"
      ><span class="lbl">{{ t('Section') }}</span
      ><select v-model="adding.section" class="input">
        <option value="" disabled>{{ t('choose…') }}</option>
        <option v-for="s in secNames" :key="s" :value="s">{{ s }}</option>
      </select></label
    >
    <label class="col" v-if="adding.section"
      ><span class="lbl">{{ t('Option') }}</span
      ><select v-model="adding.key" class="input">
        <option value="" disabled>{{ t('choose…') }}</option>
        <option v-for="k in Object.keys(rawSection(adding.section) || {})" :key="k" :value="k">{{ k }}</option>
      </select></label
    >
    <label class="col"
      ><span class="lbl">{{ t('Unit (optional)') }}</span
      ><input v-model="adding.unit" class="input" placeholder="mm/s"
    /></label>
    <template #foot
      ><button class="btn lg" @click="adding = null">{{ t('Cancel') }}</button
      ><button class="btn lg acc" :disabled="!adding.key" @click="addField">{{ t('Add') }}</button></template
    >
  </Modal>
</template>
<style scoped>
