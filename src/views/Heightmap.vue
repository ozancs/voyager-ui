...
function color(z) {
  ...
  const t = Math.max(-1, Math.min(1, z / lim))
...
<template>
  <div class="split" style="min-height:calc(100vh - 208px)">
    <section class="card grow">
      <div class="card-h">
        <h2>Heightmap · {{ bm.profile_name || 'no mesh loaded' }}</h2>
        <div class="acts"><div class="seg" style="width:130px"><button :class="{ on: mode3d }" @click="mode3d = true">3D</button><button :class="{ on: !mode3d }" @click="mode3d = false">2D</button></div><div class="seg" style="width:180px"><button :class="{ on: useProbed }" @click="useProbed = true">Probed</button><button :class="{ on: !useProbed }" @click="useProbed = false">Mesh</button></div></div>
      </div>
      <div v-if="!matrix" class="empty" style="flex:1;display:flex;align-items:center;justify-content:center">No bed mesh loaded. Calibrate or load a profile.</div>
      ...
      <div class="mono mu" style="font-size:12px" v-if="matrix && !mode3d">front of bed is at the bottom</div>
    </section>
    <div class="side-col">
      <section class="card">
        <div class="card-h"><h2>Mesh</h2></div>
        <div v-if="stats" class="st">
          <div><span class="lbl">Max</span><b class="mono" style="color:var(--ac)">{{ stats.mx.toFixed(3) }}</b></div>
          <div><span class="lbl">Min</span><b class="mono" style="color:var(--bl)">{{ stats.mn.toFixed(3) }}</b></div>
          <div><span class="lbl">Range</span><b class="mono">{{ stats.range.toFixed(3) }}</b></div>
          <div><span class="lbl">Points</span><b class="mono">{{ stats.cols }}×{{ stats.rows }}</b></div>
        </div>
        <div class="g2">
          <button class="btn lg acc" :disabled="isPrinting" @click="gcode('BED_MESH_CALIBRATE')"><Icon name="mesh" :stroke="2.4" />Calibrate</button>
          <button class="btn lg" :disabled="!bm.profile_name" @click="gcode('BED_MESH_CLEAR')"><Icon name="x" :stroke="2.4" />Clear</button>
        </div>
      </section>
      <section class="card">
        <div class="card-h"><h2>Profiles</h2><button class="btn" :disabled="!matrix" @click="showSave = true"><Icon name="save" :size="16" />Save as</button></div>
        <div v-if="!profiles.length" class="empty">No saved profiles</div>
        <div v-for="p in profiles" :key="p" class="pr">
          <b class="grow">{{ p }}</b>
          <span v-if="p === bm.profile_name" class="chip" style="color:var(--ok)"><i></i>Active</span>
          <button v-else class="btn" @click="gcode(`BED_MESH_PROFILE LOAD=${p}`)">Load</button>
          <button class="btn clear ibtn sm" aria-label="Remove" @click="gcode(`BED_MESH_PROFILE REMOVE=${p}`)"><Icon name="trash" :size="16" /></button>
        </div>
        <span class="mu" style="font-size:12px">Removing or saving a profile needs SAVE_CONFIG to persist.</span>
      </section>
      <section class="card">
        <div class="card-h"><h2>View</h2></div>
        <div class="row" style="justify-content:space-between"><span>Fixed scale ±0.1 mm</span><button class="toggle" :class="{ on: flat }" @click="flat = !flat" aria-label="Fixed scale"><span></span></button></div>
      </section>
    </div>
  </div>
  <Modal v-if="showSave" title="Save mesh profile" @close="showSave = false">
    <input v-model="saveName" class="input" placeholder="profile name" aria-label="Profile name" @keydown.enter="doSave" />
    <template #foot><button class="btn lg" @click="showSave = false">Cancel</button><button class="btn lg acc" @click="doSave">Save</button></template>
  </Modal>
</template>
<style scoped>
