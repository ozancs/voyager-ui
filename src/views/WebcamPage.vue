...
<template>
  <div class="split" style="min-height:calc(100vh - 208px)">
    <section class="card grow" ref="wrap">
      <div class="card-h"><h2>{{ cam?.name || 'Webcam' }}</h2><div class="acts"><button class="btn" :disabled="!cam" @click="snapshot"><Icon name="snap" :size="16" :stroke="2.4" />Snapshot</button><button class="btn" @click="full"><Icon name="ext" :size="16" :stroke="2.4" />Fullscreen</button></div></div>
      <WebcamView :cam="cam" />
    </section>
    <div class="side-col">
      <section class="card">
        <div class="card-h"><h2>Cameras</h2></div>
        <div v-if="!state.webcams.length" class="empty">No webcams. Add one in Mainsail or moonraker.conf ([webcam] section).</div>
      ...
      <section v-if="cam" class="card">
        <div class="card-h"><h2>Stream</h2></div>
        <div class="kv"><span class="mu">Stream</span><span class="mono">{{ cam.stream_url }}</span></div>
        <div class="kv"><span class="mu">Snapshot</span><span class="mono">{{ cam.snapshot_url }}</span></div>
        <div class="kv"><span class="mu">Flip / rotate</span><span class="mono">{{ cam.flip_horizontal ? 'H ' : '' }}{{ cam.flip_vertical ? 'V ' : '' }}{{ cam.rotation || 0 }}°</span></div>
      </section>
    </div>
  </div>
</template>
