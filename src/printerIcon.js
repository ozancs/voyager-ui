...
export function readIcon(file) {
  return new Promise((res, rej) => {
    ...
    r.onload = () => {
      ...
      im.onload = () => {
        const S = 128,
          iw = im.naturalWidth || S,
          ih = im.naturalHeight || S,
          k = S / Math.max(iw, ih); // any size or shape ends up fitted into the square
...
export function startTabIcon() {
  ...
  watch(
    () => [iconUrl(), printerName.value],
    ([u]) => {
      if (u) {
        link.setAttribute('href', u);
        link.setAttribute('type', /^data:image\/svg|\.svg(\?|$)/.test(u) ? 'image/svg+xml' : 'image/png');
      } else {
        link.setAttribute('href', orig.href);
        link.setAttribute('type', orig.type);
      }
    },
    { immediate: true },
  );
