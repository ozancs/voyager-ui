...
export function lintKlipper(doc, ctx = {}) {
  ...
  const pos = []; let acc = 0
  for (const l of lines) { pos.push(acc); acc += l.length + 1 }
  const at = (i, sev, msg, col = 0, len) => diags.push({ from: pos[i] + col, to: pos[i] + (len != null ? col + len : lines[i].length), severity: sev, message: msg })
  let section = null, secLine = -1, keys = {}, block = null
  const sectionLines = {} // lowercase section -> line index
  const optionLines = {}  // "section|option" -> line index
  const closeBlock = () => {
    if (!block) return
    for (const [open, close] of [['if', 'endif'], ['for', 'endfor']]) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const l = lines[i]
    if (/^#\*#/.test(l)) { closeBlock(); break } // SAVE_CONFIG block, written by Klipper
    const sm = l.match(/^\[([^\]]+)\]/)
    if (sm) {
      closeBlock()
      section = sm[1].trim(); secLine = i; keys = {}
    ...
    if (km) {
      ...
      if (!section) { at(i, 'error', 'Option outside of any [section]'); continue }
  ...
  for (const w of ctx.warnings || []) {
    const i = optionLines[(w.section || '').toLowerCase() + '|' + (w.option || '').toLowerCase()] ?? sectionLines[(w.section || '').toLowerCase()]
    if (i != null) at(i, 'warning', w.message || `${w.option} is deprecated`)
  }
  const e = ctx.klippyError || ''
  let m
  if ((m = e.match(/Option '([^']+)' in section '([^']+)'/i)) || (m = e.match(/option '([^']+)' is not valid in section '([^']+)'/i))) {
