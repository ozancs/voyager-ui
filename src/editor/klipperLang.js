...
export const klipperStream = StreamLanguage.define({
  ...
  token(stream, st) {
    ...
    if (sol) {
      st.lineStart = true
      // section header
      if (stream.match(/^\[[^\]]*\]/)) { st.section = stream.current().slice(1, -1); st.key = ''; st.gcode = false; st.lineStart = false; return 'heading' }
      // SAVE_CONFIG block marker lines start with #*#
      if (stream.match(/^#\*#.*/)) return 'meta'
      if (stream.match(/^[#;].*/)) return 'comment'
      // option at column 0
      const m = stream.match(/^[A-Za-z0-9_.\-]+(?=\s*[:=])/)
      if (m) { st.key = stream.current(); st.gcode = GCODE_KEY.test(st.key); st.afterKey = true; st.lineStart = false; return 'propertyName' }
    }
    if (st.afterKey) {
      if (stream.match(/^\s*[:=]/)) { st.afterKey = false; st.valueStart = true; return 'punctuation' }
      st.afterKey = false
    }
    ...
    if (inGcode) {
      if (stream.match(/^\{%-?/)) { st.jinja = 1; return 'keyword' }
      if (stream.match(/^-?%\}/)) { st.jinja = 0; return 'keyword' }
      if (stream.match(/^\{\{|^\}\}/)) return 'meta'
      if (st.jinja) {
        if (stream.match(/^(if|elif|else|endif|for|endfor|in|set|macro|endmacro|not|and|or|is|filter|endfilter)\b/)) return 'keyword'
        if (stream.match(/^(printer|params|rawparams|action_respond_info|action_raise_error|action_emergency_stop|action_call_remote_method)\b/)) return 'variableName.special'
    ...
    if (stream.match(/^[!^~]*(PA|PB|PC|PD|PE|PF|PG|P\d|gpio)\d+\b/i) || stream.match(/^[!^~]*[a-z0-9_]+:[A-Za-z0-9_]+/i)) return 'atom'
    stream.next()
    return 'string'
  },
