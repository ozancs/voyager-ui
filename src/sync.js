...
export async function detect() {
  try {
    ...
    sync.mainsail = sync.fluidd = false;
  }
  sync.checked = true;
}
...
async function get(namespace, key) {
  try {
    return (await api.call('server.database.get_item', { namespace, key })).value;
  } catch {
    return undefined;
  }
}
...
function heaterType(obj) {
  return obj === 'extruder' || obj.startsWith('extruder') || obj === 'heater_bed'
    ? 'heater'
...
function findHeater(name) {
  const all = [
    ...(S('heaters').available_heaters || []),
    ...(S('heaters').available_sensors || []).filter((s) => s.startsWith('temperature_fan')),
  ];
  ...
  return presets.map((p) => ({
    ...
    values: Object.fromEntries(
      Object.entries(p.temps || {}).map(([h, v]) => [
        h,
        { type: heaterType(h), bool: Number(v) > 0, value: Number(v) || 0 },
      ]),
    ),
  }));
}
function toFluiddPresets(presets) {
  return presets.map((p, i) => ({
    ...
    values: Object.fromEntries(
      Object.entries(p.temps || {}).map(([h, v]) => [
        h,
        { type: heaterType(h) === 'temperature_fan' ? 'fan' : 'heater', active: Number(v) > 0, value: Number(v) || 0 },
      ]),
    ),
  }));
}
...
function shared() {
  ...
  const out = [
    ...
    [
      null,
      'uiSettings.general.toolheadMoveDistances',
      asc([...new Set([...(num(c.stepsXY) || []), ...(num(c.stepsZ) || []), ...(num(c.dpad) || [])])]),
    ],
    ...
    [
      null,
      'uiSettings.general.defaultExtrudeLength',
      asc(c.extAmounts)[Math.floor((asc(c.extAmounts).length || 1) / 2)] || 25,
    ],
    [
      null,
      'uiSettings.general.defaultExtrudeSpeed',
      asc(c.extFeeds)[Math.floor((asc(c.extFeeds).length || 1) / 2)] || 5,
    ],
    ...
    .map((p, i) => ({
      ...
      temps: Object.fromEntries(
        Object.entries(p.values || {}).map(([h, v]) => [
          findHeater(h) || h,
          v?.bool === false ? 0 : Number(v?.value) || 0,
        ]),
      ),
    }));
}
  ...
  return (list || [])
    ...
    .map((p, i) => ({
      ...
      temps: Object.fromEntries(
        Object.entries(p.values || {}).map(([h, v]) => [
          findHeater(h) || h,
          v?.active === false ? 0 : Number(v?.value) || 0,
        ]),
      ),
    }));
}
...
export async function importFrom(which) {
  ...
  const took = [];
  if (which === 'mainsail') {
    const g = (await get('mainsail', 'general')) || {},
      ct = (await get('mainsail', 'control')) || {},
      pr = (await get('mainsail', 'presets')) || {};
    if (g.printername) {
      s.printerName = g.printername;
      took.push('name');
    }
    ...
    const ui = (await get('fluidd', 'uiSettings')) || {},
      g = ui.general || {};
    if (g.instanceName) {
      s.printerName = g.instanceName;
      took.push('name');
    }
    ...
    if (num(g.toolheadMoveDistances)?.length) {
      const d = num(g.toolheadMoveDistances);
      c.stepsXY = d.filter((x) => x >= 1).slice(-3);
      c.stepsZ = d.filter((x) => x <= 25).slice(0, 3);
      c.dpad = d;
    }
...
export async function initSync() {
  ...
  if (!state.settings.syncImported && state.settings.sync !== false && (sync.mainsail || sync.fluidd)) {
    try {
      await importFrom(sync.mainsail ? 'mainsail' : 'fluidd');
    } catch {}
