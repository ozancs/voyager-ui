...
async function run(cmd) {
  if (armed.value !== cmd) {
    armed.value = cmd;
    setTimeout(() => {
      if (armed.value === cmd) armed.value = '';
    }, 4000);
    return;
  }
  armed.value = '';
...
const lanes = computed(() => {
  ...
  return state.objects
    .filter((o) => /^AFC_(stepper|lane) /.test(o))
    .map((o, i) => {
      const l = S(o),
        name = l.name || o.split(' ').slice(1).join(' ');
      const ready = l.prep && l.load;
      return {
        id: name,
        label: name.replace(/^lane/i, ''),
        status: l.tool_loaded ? 1 : ready ? 1 : l.prep ? 2 : 0,
        color: rgb(l.color),
        material: l.material || '',
        weight: l.weight,
        spoolId: l.spool_id,
        tools: l.map ? [String(l.map).replace(/^T/i, '')] : [],
        current: cur === name || l.tool_loaded,
        loaded: !!l.tool_loaded,
        unit: l.unit,
        lstatus: l.status,
      };
    });
...
function gateActions(g) {
  ...
  if (kind.value === 'hh') {
    for (const tool of g.tools)
      out.push({ label: t('Load T{n}', { n: tool }), cmd: `MMU_CHANGE_TOOL TOOL=${tool}`, need: 'MMU_CHANGE_TOOL' });
...
const mainActions = computed(() => {
  if (kind.value === 'hh') {
    ...
    return [
      ['MMU_LOAD', 'Load', 'load'],
      ['MMU_UNLOAD', 'Unload', 'unload'],
      ['MMU_EJECT', 'Eject', 'up'],
      ['MMU_HOME', 'Home', 'home'],
      ['MMU_CHECK_GATE ALL=1', 'Check all gates', 'search'],
      ['MMU_RECOVER', 'Recover', 'refresh'],
  ...
  return [
    ['TOOL_UNLOAD', 'Unload', 'unload'],
    ['AFC_CALIBRATION', 'Calibrate', 'target'],
  ].filter(([c]) => has(c));
});
// editing what is loaded in a gate / lane
const editGate = ref(null);
const canSetFilament = computed(() =>
...
<template>
  <section class="card mmu">
    <div class="card-h">
      ...
      <div class="acts">
        <span v-if="kind === 'hh'" class="chip" :style="{ color: busy ? 'var(--wn)' : 'var(--mu)' }"
          ><i></i>{{ m.enabled === false ? t('Disabled') : t(m.action || 'Idle') }}</span
        >
        <span v-if="kind === 'hh' && m.sync_drive" class="chip" :data-tip="t('Gear motor synced to the extruder')"
          >sync</span
        >
      </div>
    </div>
    ...
    <template v-else>
      <div class="now">
        <div class="big">
          <span class="lbl">{{ t('Tool') }}</span
          ><b>{{ kind === 'hh' ? hhTool : current ? 'T' + (current.tools[0] ?? '?') : '-' }}</b>
        </div>
        <div class="big">
          <span class="lbl">{{ kind === 'hh' ? t('Gate') : t('Lane') }}</span
          ><b>{{ current?.label ?? (kind === 'hh' && m.gate === -2 ? t('Bypass') : '-') }}</b>
        </div>
        <div class="big">
          <span class="lbl">{{ t('Filament') }}</span
          ><b
            :style="{ color: (kind === 'hh' ? m.filament === 'Loaded' : current?.loaded) ? 'var(--tx)' : 'var(--mu)' }"
            >{{ kind === 'hh' ? t(m.filament || 'Unknown') : current?.loaded ? t('Loaded') : t('Unloaded') }}</b
          >
        </div>
      ...
      <div v-if="kind === 'hh' && m.reason_for_pause && ['paused', 'pause_locked'].includes(m.print_state)" class="why">
        <Icon name="warn" :size="15" />{{ m.reason_for_pause }}
      </div>
      ...
      <div class="gates">
        <div
          ...
          :key="g.id"
          class="gate"
          :class="{ cur: g.current, gempty: g.status === 0, open: open === g.id }"
        >
          <button
            class="gb"
            :aria-label="(kind === 'hh' ? t('Gate') : t('Lane')) + ' ' + g.label"
            @click.stop="open = open === g.id ? null : g.id"
          >
            ...
            <i
              class="st"
              :style="{ background: (STATUS[g.status] || STATUS[-1])[1] }"
              :aria-label="t((STATUS[g.status] || STATUS[-1])[0])"
            ></i>
          </button>
        </div>
      </div>
      ...
      <div v-if="openGate" class="gm" @click.stop>
        <div class="row" style="justify-content: space-between">
          <b>{{
            openGate.name || openGate.material || (kind === 'hh' ? t('Gate {n}', { n: openGate.label }) : openGate.id)
          }}</b
          ><span class="mu sm"
            >{{ t((STATUS[openGate.status] || STATUS[-1])[0])
            }}<template v-if="openGate.temp"> · {{ openGate.temp }}°</template
            ><template v-if="openGate.weight"> · {{ Math.round(openGate.weight) }} g</template></span
          >
        </div>
        <button
          v-for="a in gateActions(openGate)"
          :key="a.cmd"
          class="btn"
          :class="{ acc: armed === a.cmd }"
          :disabled="isPrinting"
          @click="run(a.cmd)"
        >
          {{ armed === a.cmd ? t('Press again to run') : a.label }}
        </button>
        <button
          v-if="canSetFilament"
          class="btn"
          ...
          "
        >
          <Icon name="pencil" :size="14" />{{ t('Set filament') }}
        </button>
        <span v-if="!gateActions(openGate).length && !canSetFilament" class="mu sm">{{
          t('No commands available for this unit.')
        }}</span>
      </div>
      <div class="acts2">
        <button
          v-for="[c, l, ic] in mainActions"
          :key="c"
          class="btn"
          :class="{ acc: armed === c }"
          :disabled="isPrinting && !/RECOVER|UNLOCK/.test(c)"
          @click="run(c)"
        >
...
.spool.none {
  box-shadow: inset 0 0 0 2px var(--mu2);
  background-image: repeating-linear-gradient(
    45deg,
    transparent 0 4px,
    color-mix(in srgb, var(--mu2) 40%, transparent) 4px 6px
  ) !important;
}
.gn {
  font-size: 14px;
