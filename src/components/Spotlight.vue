...
const PAGES = [
  ...
  ['machine', 'cpu', 'Machine'],
  ['health', 'heart', 'Health & Maintenance'],
  ['quick', 'sliders', 'Printer settings'],
  ['config', 'code', 'Config files'],
];
const SETTINGS = [
  ['general', 'Interface settings', 'settings ayarlar arayüz interface'],
  ['general', 'Language', 'dil türkçe english turkish'],
  ['general', 'Printer name', 'name isim'],
  ['dashboard', 'Dashboard settings', 'customize layout cards sensors chart favorites'],
  ['control', 'Control: jog speeds and steps', 'toolhead move increments extrude z offset invert'],
  ['presets', 'Temperature presets', 'pla petg abs preheat'],
  ['console', 'Console settings', 'hide temperatures'],
  ['appearance', 'Appearance', 'theme colour accent dark light scale size menu'],
  ['sounds', 'Sounds & alerts', 'sound audio beep volume error popup toast notification dashboard printing'],
  ['notify', 'Phone notifications', 'telegram discord ntfy pushover apprise notifier bildirim push'],
  ['webcams', 'Webcams', 'camera kamera crowsnest stream'],
  ['sync', 'Mainsail / Fluidd sync', 'shared settings import'],
...
const ACTIONS = computed(() => [
  ...
  {
    t: 'Save config',
    s: 'SAVE_CONFIG, restarts Klipper',
    icon: 'save',
    run: () => gcode('SAVE_CONFIG'),
    dest: 'Runs SAVE_CONFIG',
  ...
  {
    t: state.settings.sound?.enabled ? 'Turn sounds off' : 'Turn sounds on',
    s: 'audio alerts mute',
    icon: state.settings.sound?.enabled ? 'mute' : 'volume',
    run: () => {
      state.settings.sound.enabled = !state.settings.sound.enabled;
      if (state.settings.sound.enabled) playSound('heated');
    },
    dest: 'Setting',
  },
  { t: 'Job queue', s: 'queue next print jobs', icon: 'queue', run: () => go('files'), dest: 'G-code Files' },
  {
    t: 'Show / hide the side menu',
    s: 'sidebar navigation menu yan menü',
    icon: 'sidebar',
    run: () => {
      state.settings.navMode = state.settings.navMode === 'hidden' ? 'pinned' : 'hidden';
    },
    dest: 'Setting',
  },
  {
    t: 'Switch light / dark theme',
    s: 'theme tema light dark açık koyu mode',
    icon: 'contrast',
    run: () => {
  ...
  {
    t: 'Edit favorites bar',
    s: 'favorite macros buttons toolbar',
    icon: 'star',
    run: () => {
      state.favEdit = true;
    },
  ...
  for (const t of tokens) {
    ...
    if (i < 0) {
      ...
      let j = 0,
        first = -1,
        last = -1;
      for (let k = 0; k < hay.length && j < t.length; k++)
        if (hay[k] === t[j]) {
          if (first < 0) first = k;
          last = k;
          j++;
...
const smart = computed(() => {
  const r = smartResults(q.value);
  const raw = rawCommand(q.value);
  return raw ? [...r, raw] : r;
});

const macroDef = (m) =>
  cfgIndex.items.find((c) => c.kind === 'section' && c.section.toLowerCase() === 'gcode_macro ' + m.toLowerCase());
function openCfg(file, line) {
  state.jump = { file, line, t: Date.now() };
  go('config', file);
}
...
const results = computed(() => {
  ...
  const add = (item, hay, bonus = 0, strict = false) => {
    if (!tokens.length) {
      if (item.cat === t('Page') || item.cat === t('Action')) out.push({ ...item, sc: bonus });
      return;
    }
    const sc = score(hay.toLowerCase(), tokens, strict);
    if (sc >= 0) out.push({ ...item, sc: sc + bonus });
  };
  for (const [k, i, l] of PAGES)
    add({ t: t(l), icon: i, cat: t('Page'), dest: t('Page'), run: () => go(k) }, l + ' ' + t(l) + ' ' + k, 12);
  for (const a of ACTIONS.value)
    if (!a.off)
      add(
        { ...a, t: t(a.t), s: t(a.s), dest: t(a.dest), cat: t('Action') },
        a.t + ' ' + t(a.t) + ' ' + a.s + ' ' + t(a.s),
        10,
      );
  for (const [id, l, kw] of SETTINGS)
    add(
      {
        t: t(l),
        icon: 'gear',
  ...
  for (const m of macroList.value) {
    ...
    add(
      {
        t: m,
        s: S('gcode_macro ' + m).description || state.commands[m] || '',
        icon: 'play',
        cat: t('Macro'),
        dest: t('Runs macro'),
        run: () => gcode(m),
        confirm: true,
        alt: def && { label: `${def.file}:${def.line}`, run: () => openCfg(def.file, def.line) },
      },
      m + ' ' + (state.commands[m] || ''),
      8,
    );
  ...
  for (const d of state.power || []) {
    ...
    add(
      {
        t: off
          ? t('Turn off {name}', { name: prettyName(d.device) })
          : t('Turn on {name}', { name: prettyName(d.device) }),
        s: t('Power device, now {state}', { state: d.status }),
        icon: 'power',
        cat: t('Power devices'),
        dest: t('Switches power'),
        run: () => flipPower(d, !off),
        confirm: true,
      },
      d.device + ' power plug ' + (off ? 'off' : 'on'),
      7,
    );
  }
  if (tokens.length) {
      ...
      add(
        {
          ...
          icon: 'term',
          cat: t('Command'),
          dest: t('Opens console'),
          run: () => {
            state.consoleDraft = k + ' ';
            go('console');
          },
        },
        k + ' ' + d,
        ...
        add(
          {
            ...
            icon: 'file',
            cat: t('Config'),
            dest: t('{file} · line {line}', { file: c.file, line: c.line }),
            run: () => openCfg(c.file, c.line),
          },
          c.section + ' ' + c.file,
          6,
        );
    ...
    for (const f of gfiles.value)
      add(
        {
          t: f.split('/').pop(),
          s: f.includes('/') ? f.split('/').slice(0, -1).join('/') : '',
          icon: 'cube',
          cat: t('G-code file'),
          dest: t('G-code Files'),
          run: () => {
...
function key(e) {
  if (e.key === 'Escape') close();
    ...
    e.preventDefault();
    idx.value = Math.min(results.value.length - 1, idx.value + 1);
    scroll();
    ...
    e.preventDefault();
    run(results.value[idx.value], e.shiftKey);
  }
}
function scroll() {
  ...
  <Transition name="sp">
    <div v-if="state.spotlight" class="ov" @mousedown.self="close">
      <div class="box" role="dialog" :aria-label="t('Search')">
        <div class="inp">
          <Icon name="search" :size="22" /><input
            ref="inp"
            v-model="q"
            :placeholder="t('Search pages, macros, config, files… or “extruder 250”')"
            :aria-label="t('Search')"
            @keydown="key"
          /><kbd>esc</kbd>
        </div>
        <div ref="listEl" class="res">
          <template v-for="(r, i) in results" :key="r.cat + r.t + i">
            ...
            <div
              class="it"
              :class="{ on: i === idx, dis: r.disabled, arm: armed === keyOf(r) }"
              @mousemove="idx !== i && (idx = i)"
              @click="run(r)"
            >
              <span class="ic"><Icon :name="r.icon" :size="17" /></span>
              <div class="tx">
                <b><i v-if="r.swatch" class="sw" :style="{ background: r.swatch }"></i>{{ r.t }}</b
                ><span v-if="r.warn" class="wn">{{ r.warn }}</span
                ><span v-else-if="r.s" class="mu">{{ r.s }}</span>
              </div>
              <button v-if="r.alt" class="alt code" :title="t('Open definition')" @click.stop="run(r, true)">
                {{ r.alt.label }}
              </button>
              <span v-if="armed === keyOf(r)" class="dest cf"
                ><Icon name="enter" :size="14" />{{ t('Press again to run') }}</span
              >
              <span v-else class="dest">{{ r.dest }}<Icon v-if="i === idx" name="enter" :size="14" /></span>
            </div>
          </template>
          <div v-if="!results.length" class="empty">
            {{ cfgIndex.loading ? t('Indexing config files…') : t('Nothing found') }}
          </div>
        </div>
        <div class="foot mu">
          <span><kbd>↑</kbd><kbd>↓</kbd> {{ t('select') }}</span
          ><span><kbd>↵</kbd> {{ t('Open') }}</span
          ><span><kbd>⇧</kbd><kbd>↵</kbd> {{ t('macro definition') }}</span
...
.ov {
  ...
  padding: calc(12vh / var(--zoom, 1)) 16px 16px;
}
.box {
  width: 680px;
  max-width: 100%;
  ...
  overflow: hidden;
  max-height: calc(70vh / var(--zoom, 1));
}
.inp {
  display: flex;
  align-items: center;
...
.it.arm {
  background: color-mix(in srgb, var(--wn) 14%, var(--s2));
  box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--wn) 45%, transparent);
}
.dest.cf {
