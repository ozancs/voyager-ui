...
const PAGES = [
  ...
  ['health', 'heart', 'Health & Maintenance'], ['quick', 'sliders', 'Quick Config'], ['theme', 'gear', 'Settings'],
]
const SETTINGS = [
  ['set-language', 'Language', 'dil türkçe english turkish'], ['set-presets', 'Temperature presets', 'pla petg abs preheat'],
...
const results = computed(() => {
  ...
  for (const [id, l, kw] of SETTINGS) add({ t: t(l), icon: 'gear', cat: t('Setting'), dest: t('Settings'), run: () => { state.anchor = id; go('theme') } }, l + ' ' + t(l) + ' ' + kw, 4)
  for (const m of macroList.value) {
    const def = macroDef(m)
    add({ t: m, s: S('gcode_macro ' + m).description || state.commands[m] || '', icon: 'play', cat: t('Macro'), dest: t('Runs macro'), run: () => gcode(m), alt: def && { label: `${def.file}:${def.line}`, run: () => openCfg(def.file, def.line) } }, m + ' ' + (state.commands[m] || ''), 8)
  }
  if (tokens.length) {
    ...
    for (const c of cfgIndex.items) {
      ...
      else add({ t: `${c.key}: ${c.value}`, s: `[${c.section}]`, icon: 'sliders', cat: t('Config'), dest: t('{file} · line {line}', { file: c.file, line: c.line }), run: () => openCfg(c.file, c.line) }, c.key + ' ' + c.section, 0)
    }
      ...
      <div class="box" role="dialog" :aria-label="t('Search')">
        ...
        <div ref="listEl" class="res">
          <template v-for="(r, i) in results" :key="r.cat + r.t + i">
            ...
            <div class="it" :class="{ on: i === idx, dis: r.disabled }" @mousemove="idx = i" @click="run(r)">
