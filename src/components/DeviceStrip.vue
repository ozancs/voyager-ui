...
const RANK = (d) =>
  d.kind === 'temp'
    ? d.obj.startsWith('temperature_fan ')
      ? 2
      : canTarget(d)
        ? 0
        : 1
    : ({ fan: 2, pin: 3, led: 3, filament: 4, spoolman: 5 }[d.kind] ?? 6);
...
onMounted(() => {
  ...
  if (box.value) ro.observe(box.value.$el || box.value);
});
...
const isHidden = (d) =>
  (state.settings.strip.hidden || []).includes(d.id) || state.settings.devices.hidden.includes(d.obj);
...
function startRename(d) {
  renaming.value = { obj: d.obj, value: state.settings.devices.names?.[d.obj] || '' };
}
function saveRename() {
...
function moveTo(id, targetId) {
  const ids = items.value
    .map((x) => x.id)
    .concat(stripAll.value.map((x) => x.id).filter((i) => !items.value.some((y) => y.id === i)));
...
function local(el, e) {
  const par = el.offsetParent || el.parentElement,
    ...
    k = r.width / (par.offsetWidth || r.width) || 1;
...
function pMove(e) {
  ...
  if (!slots.length)
    slots = [...dragEl.parentElement.children]
      .filter((x) => x.classList?.contains('dc'))
      .map((x) => ({ id: x.dataset.id, x: x.offsetLeft, y: x.offsetTop, w: x.offsetWidth, h: x.offsetHeight }));
  const px = q.x,
    py = q.y;
...
function pUp() {
  ...
  if (dragId.value && overId.value) {
    settle.value = true;
    moveTo(dragId.value, overId.value);
    requestAnimationFrame(() => requestAnimationFrame(() => (settle.value = false)));
  }
  dragStart = null;
  dragEl = null;
  slots = [];
...
function tint(d) {
  ...
  return d.kind === 'temp' && (S('heaters').available_heaters || []).includes(d.obj) && S(d.obj).target > 0
    ? 't-heat'
    : '';
}
...
function toggleOpen(id, e) {
  ...
  if (e?.currentTarget) {
    ...
    popRight.value = r.left + r.width / 2 > window.innerWidth / 2;
  }
...
function setLed(id, hex, white) {
  ...
  gcode(
    `SET_LED LED=${shortName(id)} RED=${r.toFixed(3)} GREEN=${g.toFixed(3)} BLUE=${b.toFixed(3)}${w} SYNC=0 TRANSMIT=1`,
  );
}
function ledToggle(id, on) {
  if (on) setLed(id, lastColor[id] || '#ffffff');
  else {
    lastColor[id] = ledHex(id);
    gcode(`SET_LED LED=${shortName(id)} RED=0 GREEN=0 BLUE=0 WHITE=0 SYNC=0 TRANSMIT=1`);
  }
}
const isPwm = (id) => {
...
function level(d) {
  ...
  else if (d.kind === 'filament')
    v = o.filament_detected || (o.filament_detected === undefined && o.enabled !== false) ? 1 : 0;
  else if (d.kind === 'spoolman' && spool.value)
    v = (spool.value.remaining_weight || 0) / (spool.value.initial_weight || spool.value.filament?.weight || 1000);
  return Math.max(0, Math.min(1, v || 0));
}
function sensorExtra(id) {
  const s = S(id);
  return Object.entries(s)
    .filter(([k, v]) => typeof v !== 'object' && !['enabled', 'filament_detected'].includes(k))
    .slice(0, 2);
}
</script>
<template>
  <TransitionGroup
    ...
    ref="box"
    class="ds"
    :class="{ editing: state.editDash, settle }"
    :style="cols ? { gridTemplateColumns: `repeat(${cols}, minmax(0, 144px))` } : null"
    move-class="mv"
  >
    <div
      v-for="(d, gi) in items"
      ...
      :class="[
        tint(d),
        {
          ...
          open: open === d.id,
          edit: state.editDash,
          hid: state.editDash && isHidden(d),
          drag: dragId === d.id,
          over: overId === d.id,
          wide: d.kind === 'temp' && canTarget(d),
          hot: d.kind === 'temp' && S(d.obj).target > 0,
        },
      ]"
      :style="{
        '--dx': dragId === d.id && state.editDash ? dragPos.x + 'px' : overId === d.id ? swapOff.x + 'px' : '0px',
        '--dy': dragId === d.id && state.editDash ? dragPos.y + 'px' : overId === d.id ? swapOff.y + 'px' : '0px',
        '--lvl': level(d),
        '--lc': d.kind === 'led' && ledOn(d.obj) ? ledHex(d.obj) : null,
      }"
      :data-id="d.id"
      @pointerdown="pDown(d, $event)"
      @click.stop="
        state.editDash
          ? null
          : (d.kind === 'temp' && canTarget(d)) ||
              (d.kind === 'fan' && d.controllable) ||
              d.kind === 'led' ||
              (d.kind === 'pin' && isPwm(d.obj))
            ? toggleOpen(d.id, $event)
            : null
      "
    >
      <div v-if="state.editDash" class="etools">
        <button
          v-if="d.kind !== 'spoolman'"
          class="btn clear ibtn sm"
          :aria-label="t('Rename card')"
        ...
        <button
          class="btn clear ibtn sm"
          :aria-label="isHidden(d) ? t('Show card') : t('Hide card')"
          @click.stop="toggleHide(d)"
        >
          <Icon :name="isHidden(d) ? 'eyeoff' : 'eye'" :size="16" />
        </button>
      </div>
      <!-- TEMP -->
      <template v-if="d.kind === 'temp'">
        <div class="hd">
          ...
          ><span v-if="canTarget(d) && !state.editDash" class="mono tt" :class="{ on: S(d.obj).target > 0 }">{{
            S(d.obj).target > 0 ? '→ ' + S(d.obj).target.toFixed(0) + '°' : t('off')
          }}</span>
        </div>
        <div class="row" style="gap: 4px; align-items: baseline">
          <span class="tbig">{{ S(d.obj).temperature != null ? S(d.obj).temperature.toFixed(1) : '--' }}</span
          ><small class="mu" style="font-size: 15px">°C</small>
        </div>
        <div class="bar">
          <div
            :style="{
              width:
                (S(d.obj).power != null ? S(d.obj).power * 100 : S(d.obj).speed != null ? S(d.obj).speed * 100 : 0) +
                '%',
              background: S(d.obj).target > 0 ? 'var(--heat)' : 'var(--mu2)',
            }"
          ></div>
        </div>
        <div v-if="open === d.id" class="pop card" :class="{ rt: popRight }" @click.stop>
          ...
          <div class="row">
            <input
              ...
              type="number"
              v-model="tgt"
              :placeholder="String(S(d.obj).target ?? 0)"
              @keydown.enter="setT(d, tgt)"
              :aria-label="t('Target temperature')"
            /><button class="btn acc" style="height: 40px" @click="setT(d, tgt)">{{ t('Set') }}</button>
          </div>
          <div class="seg">
      ...
      <template v-else-if="d.kind === 'fan'">
        <div class="hd">
          <span class="lbl nm">{{ prettyName(d.obj) }}</span
          ><span v-if="d.auto" class="auto"><Icon name="lock" :size="11" :stroke="2.6" />{{ t('auto') }}</span>
        </div>
        ...
        <div v-if="open === d.id" class="pop card" :class="{ rt: popRight }" @click.stop>
          <RangeSlider
            :label="prettyName(d.obj)"
            :model-value="pct(S(d.obj).speed)"
            :display="pct(S(d.obj).speed) + '%'"
            unit="%"
            @commit="setFan(d.obj, $event)"
          />
          <div class="seg">
            <button
              v-for="v in [0, 25, 50, 75, 100]"
              :key="v"
              :class="{ on: pct(S(d.obj).speed) === v }"
              @click="setFan(d.obj, v)"
            >
              {{ v ? v + '%' : t('Off') }}
            </button>
          </div>
      ...
      <template v-else-if="d.kind === 'pin'">
        ...
        <div class="row" style="justify-content: space-between">
          <Icon name="bulb" :size="24" :class="S(d.obj).value > 0 ? 'acc' : 'mu'" /><Toggle
            v-if="!isPwm(d.obj)"
            :model-value="S(d.obj).value > 0"
            :label="prettyName(d.obj)"
            @update:model-value="setPin(d.obj, $event ? 1 : 0)"
          />
        </div>
        <span class="mono sm"
          >{{ S(d.obj).value > 0 ? t('ON') : t('OFF')
          }}<template v-if="isPwm(d.obj)"> · {{ pct(S(d.obj).value) }}%</template></span
        >
        <div v-if="open === d.id" class="pop card" :class="{ rt: popRight }" @click.stop>
          <RangeSlider
            :label="prettyName(d.obj)"
            :model-value="pct(S(d.obj).value)"
            :display="pct(S(d.obj).value) + '%'"
            unit="%"
            @commit="setPin(d.obj, ($event / 100).toFixed(2))"
          />
        </div>
      </template>
      <!-- LED -->
      <template v-else-if="d.kind === 'led'">
        <span class="lbl nm">{{ prettyName(d.obj) }}</span>
        <div class="row" style="justify-content: space-between">
          <span class="sw" :style="{ background: ledOn(d.obj) ? ledHex(d.obj) : 'var(--s2)' }"></span
          ><Toggle
            :model-value="ledOn(d.obj)"
            :label="prettyName(d.obj)"
            @update:model-value="ledToggle(d.obj, $event)"
          />
        </div>
        ...
        <div v-if="open === d.id" class="pop card" :class="{ rt: popRight }" @click.stop>
          ...
          <label class="row"
            ><input
              type="color"
              :value="ledHex(d.obj)"
              @change="setLed(d.obj, $event.target.value)"
              style="width: 48px; height: 36px; border: none; background: none; padding: 0"
            /><span class="mu" style="font-size: 13px">{{ t('Custom') }}</span></label
          >
      ...
      <template v-else-if="d.kind === 'filament'">
        <div class="hd">
          <span class="lbl nm">{{ prettyName(d.obj) }}</span
          ><Toggle
            v-if="!d.custom && S(d.obj).enabled !== undefined"
            :model-value="!!S(d.obj).enabled"
            :label="t('Enable sensor')"
            @update:model-value="gcode(`SET_FILAMENT_SENSOR SENSOR=${shortName(d.obj)} ENABLE=${$event ? 1 : 0}`)"
          />
        </div>
        <div class="row" style="gap: 10px" v-if="S(d.obj).filament_detected !== undefined">
          <Icon
            ...
            :size="24"
            :stroke="2.4"
            :style="{ color: S(d.obj).filament_detected ? null : 'var(--dg)' }"
          />
          <b style="font-size: 17px" :style="{ color: S(d.obj).filament_detected ? null : 'var(--dg)' }">{{
            S(d.obj).filament_detected ? t('Detected') : t('Empty')
          }}</b>
        </div>
        <div v-else class="row" style="gap: 10px">
          <Icon name="sensor" :size="24" :stroke="2.4" class="ki" /><b style="font-size: 15px">{{
            S(d.obj).enabled === false ? t('Disabled') : t('Active')
          }}</b>
        </div>
        <span class="mono sm mu">{{
          S(d.obj).enabled === false
            ? t('disabled')
            : sensorExtra(d.obj)
        ...
        <a class="spl" :href="spoolUrl" target="_blank" rel="noopener" @click.stop>
          ...
          <div v-if="spool" class="row" style="gap: 10px; min-width: 0">
            ...
            <div class="col" style="gap: 0; min-width: 0">
              <b class="ell" style="font-size: 14px">{{ spool.filament?.name || spool.filament?.material }}</b
              ><span class="mono mu" style="font-size: 11px">{{ spool.filament?.material }} · #{{ spool.id }}</span>
            </div>
          </div>
          <div v-else class="mu" style="font-size: 13px">{{ t('No active spool') }}</div>
          <div v-if="spool" class="row">
            <div class="bar grow">
              <div
                :style="{
                  width:
                    Math.min(
                      100,
                      (spool.remaining_weight / (spool.initial_weight || spool.filament?.weight || 1000)) * 100,
                    ) + '%',
                }"
              ></div>
            </div>
            <b class="mono" style="font-size: 12px">{{ Math.round(spool.remaining_weight || 0) }} g</b>
          </div>
        </a>
      </template>
    </div>
  ...
  <Modal v-if="renaming" :title="t('Card name')" @close="renaming = null">
    <input
      ...
      class="input"
      :placeholder="prettyName(renaming.obj)"
      :aria-label="t('Card name')"
      @keydown.enter="saveRename"
    />
    <span class="mu" style="font-size: 12px">{{
      t('Empty uses the name from printer.cfg ({name}).', { name: renaming.obj })
    }}</span>
    <template #foot
      ><button class="btn lg" @click="renaming = null">{{ t('Cancel') }}</button
      ><button class="btn lg acc" @click="saveRename">{{ t('Save') }}</button></template
    >
  </Modal>
</template>
<style scoped>
.ds {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(124px, 144px));
  gap: 8px;
  flex: 1;
  min-width: 0;
}
...
.dc.hot {
  box-shadow: inset 0 0 0 1px var(--heat-ln);
}
.dc.edit .spl {
  pointer-events: none;
}
