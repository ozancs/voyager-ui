...
const go = (d) => {
  i.value = (i.value + d + props.list.length) % props.list.length;
};
const key = (e) => {
...
<template>
  <Modal :title="name" width="min(1200px, 96vw)" @close="emit('close')">
    ...
    <template #foot>
      <span v-if="list.length > 1" class="mono mu" style="font-size: 12px; margin-right: auto"
        >{{ i + 1 }} / {{ list.length }}</span
      >
      <button v-if="list.length > 1" class="btn lg ibtn" :aria-label="t('Previous')" @click="go(-1)">
        <Icon name="left" :size="20" :stroke="2.4" />
      </button>
      <button v-if="list.length > 1" class="btn lg ibtn" :aria-label="t('Next')" @click="go(1)">
        <Icon name="right" :size="20" :stroke="2.4" />
      </button>
...
.iv {
  ...
  border-radius: 10px;
  overflow: auto;
  max-height: calc(72vh / var(--zoom, 1));
}
.iv img {
  max-width: 100%;
  height: auto;
