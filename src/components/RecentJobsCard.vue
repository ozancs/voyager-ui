...
<template>
  <section class="card">
    <div class="card-h"><h2>{{ t('Recent prints') }}</h2><button class="btn clear" @click="go('history')">{{ t('History') }}</button></div>
    <div class="list">
      <div v-for="j in jobs" :key="j.job_id" class="jr">
        <span class="d" :style="{ background: COL[j.status] || 'var(--mu)' }" :title="j.status"></span>
        <div class="col grow" style="gap:1px;min-width:0"><b class="fn">{{ j.filename }}</b><span class="mono mu">{{ fmtTime(j.print_duration) }} · {{ fmtDate(j.start_time) }} · {{ t(j.status) }}</span></div>
        <button class="btn ibtn sm" :aria-label="t('Print again')" :disabled="isPrinting || !j.exists" @click="reprint(j)"><Icon name="refresh" :size="15" /></button>
      </div>
...
<style scoped>
.list { display: flex; flex-direction: column; gap: 4px; overflow: auto; min-height: 0; flex: 1; }
.jr { display: flex; align-items: center; gap: 10px; padding: 6px 4px; border-radius: 10px; }
.jr:hover { background: var(--s2); }
.d { width: 8px; height: 8px; border-radius: 4px; flex-shrink: 0; }
