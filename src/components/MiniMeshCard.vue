...
<template>
  <section class="card">
    <div class="card-h"><h2>{{ t('Bed Mesh') }}</h2><div class="acts"><button class="btn" :disabled="isPrinting" @click="gcode('BED_MESH_CALIBRATE')"><Icon name="mesh" :size="16" />{{ t('Calibrate') }}</button><button class="btn" :aria-label="t('Open heightmap')" @click="go('heightmap')"><Icon name="ext" :size="16" /></button></div></div>
    ...
    <template v-else>
      ...
      <div class="row mono" style="justify-content:space-between;font-size:12px"><span>{{ bm.profile_name }}</span><span>{{ t('range {v}', { v: (st.mx - st.mn).toFixed(3) }) }}</span></div>
    </template>
  </section>
</template>
<style scoped>
