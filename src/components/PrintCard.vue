...
const speed = computed(() =>
  active.value && mr.value.live_velocity != null ? Math.round(mr.value.live_velocity) + ' mm/s' : '--',
);
const flow = computed(() => {
...
const why = computed(() => {
  ...
  const out = [];
  if (w.slicerLeft != null)
    out.push(
      w.k
        ? t('Slicer {time} × {k} (learned from {n} prints)', {
            time: fmtTime((w.slicerLeft / (w.k || 1)) * (w.sf || 1)),
            k: w.k.toFixed(2),
            n: w.n,
          })
        : t('Slicer {time}', { time: fmtTime(w.slicerLeft * (w.sf || 1)) }),
    );
...
<template>
  <section class="card pc">
    ...
    <div class="grow col" style="gap: 10px">
      ...
      <div class="stats">
        <div>
          <span class="lbl">{{ t('Layer') }}</span
          ><b class="mono">{{ layerInfo.cur }} / {{ layerInfo.total || '--' }}</b>
        </div>
        <div>
          <span class="lbl">Z</span><b class="mono">{{ z }} mm</b>
        </div>
        <div>
          <span class="lbl">{{ t('Speed') }}</span
          ><b class="mono">{{ speed }}</b>
        </div>
        <div>
          <span class="lbl">{{ t('Flow') }}</span
          ><b class="mono">{{ flow }}</b>
        </div>
        <div>
          <span class="lbl">{{ t('Filament') }}</span
          ><b class="mono">{{ filament }}</b>
        </div>
        <div>
          <span class="lbl">{{ t('Print time') }}</span
          ><b class="mono">{{ fmtTime(ps.print_duration) }}</b>
        </div>
        <div>
          <span class="lbl">{{ t('Slicer') }}</span
          ><b class="mono">{{ slicer }}</b>
        </div>
        <div>
          <span class="lbl">{{ t('Left') }}</span
          ><b class="mono">{{ active ? fmtTime(printTimes.left) : '--' }}</b>
        </div>
        <button
          class="etab"
          :class="{ on: showWhy }"
          :disabled="!active"
          :data-tip="active ? why.join(' · ') : ''"
          :aria-label="t('How the finish time is estimated')"
          @click="showWhy = !showWhy"
        >
          <span class="lbl">{{ t('ETA') }} <Icon name="sparkle" :size="11" :stroke="2.4" /></span
          ><b class="mono">{{ active ? '~' + eta : '--' }}</b>
        </button>
      </div>
      <div v-if="showWhy && active" class="why">{{ why.join(' · ') }}</div>
      <div class="row" style="gap: 12px">
        <div class="bar grow state" :style="{ height: '12px', '--pst': color }">
          <div :style="{ width: progress * 100 + '%' }"></div>
        </div>
        <b class="mono" style="font-size: 16px">{{ (progress * 100).toFixed(1) }}%</b>
      </div>
    </div>
    <div class="acts">
      <template v-if="active">
        ...
        <button class="btn lg out" :disabled="!eo.objects?.length" @click="emit()">
      ...
      <template v-else>
        <button class="btn lg acc" @click="go('files')">
          <Icon name="file" :stroke="2.4" />{{ t('Choose file') }}
        </button>
        <button class="btn lg" :disabled="!ps.filename" @click="reprint">
          <Icon name="refresh" :stroke="2.4" />{{ t('Reprint') }}
        </button>
      </template>
    </div>
  </section>
  <Modal v-if="ask" :title="t('Cancel print?')" @close="ask = false">
    ...
    <template #foot
      ><button class="btn lg" @click="ask = false">{{ t('Keep printing') }}</button
      ><button class="btn lg dgf" @click="cancel">{{ t('Cancel print') }}</button></template
    >
  </Modal>
</template>
<style scoped>
...
.etab:disabled .lbl :deep(svg) {
  display: none;
}
.etab.on b,
.etab:hover:not(:disabled) b {
  color: var(--ac);
}
.why {
  font-size: 12px;
