...
<template>
  <section class="card spread">
    ...
    <div class="g2">
      ...
      <NumField :label="t('Acceleration')" unit="mm/s²" :model-value="th.max_accel" :step="500" :min="1" @commit="gcode(`SET_VELOCITY_LIMIT ACCEL=${$event}`)" />
      <NumField :label="t('Square corner vel.')" unit="mm/s" :model-value="th.square_corner_velocity" :step="1" :min="0" :decimals="1" @commit="gcode(`SET_VELOCITY_LIMIT SQUARE_CORNER_VELOCITY=${$event}`)" />
      <NumField v-if="hasMcr" :label="t('Min cruise ratio')" :model-value="th.minimum_cruise_ratio" :step="0.05" :min="0" :max="0.99" :decimals="2" @commit="gcode(`SET_VELOCITY_LIMIT MINIMUM_CRUISE_RATIO=${$event}`)" />
      <NumField v-else :label="t('Accel to decel')" unit="mm/s²" :model-value="th.max_accel_to_decel" :step="500" :min="1" @commit="gcode(`SET_VELOCITY_LIMIT ACCEL_TO_DECEL=${$event}`)" />
    </div>
    <div class="g2">
      <div class="fac">
        <div class="row" style="justify-content:space-between"><span class="nl">{{ t('Speed factor') }}</span><label class="nb"><input class="mono" type="number" :value="lv.speed ?? speed" @change="setSpeed($event.target.value === '' ? '' : +$event.target.value)" :aria-label="t('Speed factor')" />%</label></div>
        <div class="row"><button class="btn clear ibtn sm" :aria-label="t('Speed -5%')" @click="setSpeed(speed - 5)"><Icon name="minus" :size="16" :stroke="2.6" /></button><Rng :value="speed" :min="10" :max="300" :step="5" :label="t('Speed factor')" @live="lv.speed = $event" @commit="setSpeed($event)" /><button class="btn clear ibtn sm" :aria-label="t('Speed +5%')" @click="setSpeed(speed + 5)"><Icon name="plus" :size="16" :stroke="2.6" /></button></div>
      </div>
      <div class="fac">
        <div class="row" style="justify-content:space-between"><span class="nl">{{ t('Flow') }}</span><label class="nb"><input class="mono" type="number" :value="lv.flow ?? flow" @change="setFlow($event.target.value === '' ? '' : +$event.target.value)" :aria-label="t('Flow')" />%</label></div>
        <div class="row"><button class="btn clear ibtn sm" :aria-label="t('Flow -1%')" @click="setFlow(flow - 1)"><Icon name="minus" :size="16" :stroke="2.6" /></button><Rng :value="flow" :min="50" :max="150" :label="t('Flow')" @live="lv.flow = $event" @commit="setFlow($event)" /><button class="btn clear ibtn sm" :aria-label="t('Flow +1%')" @click="setFlow(flow + 1)"><Icon name="plus" :size="16" :stroke="2.6" /></button></div>
      </div>
    </div>
