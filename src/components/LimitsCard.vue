  ...
  <section class="card">
    ...
    <div class="g2">
      ...
      <div class="fac">
        ...
        <div class="row"><button class="btn clear ibtn sm" :aria-label="t('Flow -1%')" @click="setFlow(flow - 1)"><Icon name="minus" :size="16" :stroke="2.6" /></button><input class="rng" type="range" min="50" max="150" :value="flow" :style="{ '--p': ((flow - 50) / 100) * 100 + '%' }" @change="setFlow(+$event.target.value)" :aria-label="t('Flow')" /><button class="btn clear ibtn sm" :aria-label="t('Flow +1%')" @click="setFlow(flow + 1)"><Icon name="plus" :size="16" :stroke="2.6" /></button></div>
      </div>
    </div>
