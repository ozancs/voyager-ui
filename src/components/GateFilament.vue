...
onMounted(async () => {
  ...
  try {
    const r = await api.call('server.spoolman.proxy', {
      request_method: 'GET',
...
const spoolLabel = (s) =>
  `#${s.id} ${[s.filament?.vendor?.name, s.filament?.name].filter(Boolean).join(' ')}${s.filament?.material ? ' · ' + s.filament.material : ''}`;
...
const title = computed(
  () => (props.kind === 'hh' ? t('Gate {n}', { n: props.gate.label }) : props.gate.id) + ' · ' + t('Filament'),
);
</script>
<template>
  <Modal :title="title" width="480px" @close="emit('close')">
    <label v-if="spools && spools.length" class="col"
      ><span class="lbl">{{ t('Spoolman spool') }}</span>
      <select class="input" :value="f.spool" @change="useSpool($event.target.value)">
        <option value="">{{ t('None') }}</option>
        <option v-for="s in spools" :key="s.id" :value="s.id">{{ spoolLabel(s) }}</option>
      </select>
    </label>
    <div class="row" style="gap: 10px; align-items: flex-end">
      <label class="col grow"
        ><span class="lbl">{{ t('Material') }}</span
        ><input v-model="f.material" class="input" list="mmu-mats" placeholder="PLA"
      /></label>
      <label class="col"
        ><span class="lbl">{{ t('Colour') }}</span
        ><input v-model="f.color" type="color" class="clr"
      /></label>
    </div>
    ...
    <template v-if="kind === 'hh'">
      <div class="row" style="gap: 10px">
        <label class="col grow"
          ...
          ><input v-model="f.name" class="input" :placeholder="t('e.g. Galaxy Black')"
        /></label>
        <label class="col" style="width: 110px"
    ...
    <code class="mu sm">{{
      kind === 'hh'
        ? `MMU_GATE_MAP GATE=${gate.id} MATERIAL=${mat || 'Unknown'} COLOR=${col}`
        : `SET_MATERIAL / SET_COLOR LANE=${gate.id}`
    }}</code>
    <template #foot
      ><button class="btn lg" @click="emit('close')">{{ t('Cancel') }}</button
      ><button class="btn lg acc" :disabled="busy" @click="save">{{ t('Save') }}</button></template
    >
  </Modal>
</template>
<style scoped>
