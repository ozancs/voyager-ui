...
let tm = {}
function later(id, fn) { clearTimeout(tm[id]); tm[id] = setTimeout(fn, 250) }
function fan(d, v) { later(d.id, () => setFan(d.id, v)) }
function pin(d, v) { const sc = cfg(d.id).scale || 1; later(d.id, () => gcode(`SET_PIN PIN=${shortName(d.id)} VALUE=${((v / 100) * sc).toFixed(3)}`)) }
const pinVal = (d) => Math.round(((S(d.id).value || 0) / (cfg(d.id).scale || 1)) * 100)
const ledOn = (id) => (S(id).color_data?.[0] || []).some((x) => x > 0.001)
function led(id, on) { gcode(`SET_LED LED=${shortName(id)} RED=${on ? 1 : 0} GREEN=${on ? 1 : 0} BLUE=${on ? 1 : 0} SYNC=0 TRANSMIT=1`) }
function sensor(id, on) { gcode(`SET_FILAMENT_SENSOR SENSOR=${shortName(id)} ENABLE=${on ? 1 : 0}`) }
const ICON = { fan: 'fan', pin: 'bulb', led: 'bulb', filament: 'sensor' }
</script>

<template>
  <section class="card">
    <div class="card-h"><h2>{{ t('Devices') }}</h2></div>
    <div class="list">
