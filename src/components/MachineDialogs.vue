...
const tilt = ref(null)
let lastTilt = null
watch(() => JSON.stringify(st.value.results || {}), (j) => {
  if (lastTilt === null) { lastTilt = j; return } // skip what was there on page load
...
const tiltRows = computed(() => Object.entries(tilt.value?.results || {}).map(([k, r]) => ({ k, name: r.name || k, ...r })))
</script>
    ...
    <div class="dlg card" role="dialog" :aria-label="p.title">
      ...
      <template v-for="(it, k) in p.items" :key="k">
        ...
        <div v-else class="bg" :class="{ grp: it.type === 'group' }">
          <button v-for="(b, j) in it.buttons" :key="j" class="btn lg" :class="cls(b.color)" @click="promptRun(b)">{{ b.label }}</button>
        </div>
      </template>
      <div v-if="p.footer.length" class="ft">
        <button v-for="(b, j) in p.footer" :key="j" class="btn lg" :class="cls(b.color)" @click="promptRun(b)">{{ b.label }}</button>
      </div>
    </div>
  </div>
...
.bg .btn { flex: 1 1 0; }
.bg:not(.grp) .btn { flex-basis: 100%; }
.ft { display: flex; gap: 8px; justify-content: flex-end; flex-wrap: wrap; margin-top: 4px; }
.btn.info { background: var(--cool-bg); color: var(--cool); }
.btn.warn { background: var(--light-bg); color: var(--light); }
.zbox { background: var(--s2); border-radius: 12px; padding: 14px; display: flex; flex-direction: column; align-items: center; gap: 6px; }
.zv { font-size: 34px; letter-spacing: -.02em; }
.steps { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; }
.steps .btn { padding: 0; }
.steps .up { color: var(--cool); }
</style>
