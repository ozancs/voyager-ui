...
<template>
  <div class="pl" :class="{ compact }">
    <div v-for="d in state.power" :key="d.device" class="pd">
      ...
      <div class="col grow" style="gap:0;min-width:0"><b class="nm">{{ prettyName(d.device) }}</b><span v-if="!compact" class="mu">{{ d.type }}<template v-if="d.locked_while_printing"> · {{ t('locked while printing') }}</template><template v-if="d.status === 'error'"> · {{ t('error') }}</template></span></div>
      <Icon v-if="d.locked_while_printing && isPrinting" name="lock" :size="14" class="mu" />
      <Toggle :model-value="on(d)" :label="prettyName(d.device)" :disabled="busy === d.device || (d.locked_while_printing && isPrinting) || d.status === 'init'" @update:model-value="(v) => flip(d, v)" />
    </div>
    <div v-if="!state.power.length" class="empty">{{ t('No power devices. Add a [power] section to moonraker.conf for a smart plug or relay.') }}</div>
  </div>
</template>
<style scoped>
.pl { display: flex; flex-direction: column; gap: 2px; }
.pd { display: flex; align-items: center; gap: 12px; min-height: 48px; padding: 0 4px; border-bottom: 1px solid var(--bd); }
.pd:last-child { border-bottom: none; }
.compact .pd { min-height: 40px; }
