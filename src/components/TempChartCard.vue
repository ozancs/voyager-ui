...
const scale = computed(() => {
  ...
  for (const s of sensors.value) {
    const h = hist[s.name];
    if (!h) continue;
    for (const v of h.t.slice(-range.value)) {
      if (v < lo) lo = v;
      if (v > hi) hi = v;
    }
    for (const v of h.target.slice(-range.value))
      if (v > 0) {
        if (v < lo) lo = v;
        if (v > hi) hi = v;
      }
...
const lines = computed(() => {
  ...
  return sensors.value
    .map((s, i) => {
      ...
      const t = h.t.slice(-range.value),
        tg = h.target.slice(-range.value);
      const n = range.value,
        off = n - t.length;
    ...
    <div class="card-h">
      ...
      <div class="acts">
        <div class="seg" style="width: 180px">
          <!-- prettier-ignore -->
          <button v-for="r in [300, 600, 1200]" :key="r" :class="{ on: range === r }" @click="state.settings.tempRange = r">{{ r / 60 }}m</button>
        </div>
        <SensorPicker lines />
      </div>
    </div>
