    ...
    <div data-fit style="overflow:auto;min-height:0"><table class="tt">
      ...
      <tbody>
        <tr v-for="(s, i) in sensors" :key="s.name">
          ...
          <td>
            <input v-if="s.isHeater || s.isTempFan" class="tg mono" :class="{ on: s.target > 0 }" type="number" :value="edit[s.name] ?? s.target?.toFixed(0)" @focus="edit[s.name] = s.target?.toFixed(0)" @input="edit[s.name] = $event.target.value" @keydown.enter="$event.target.blur()" @blur="commitTarget(s, $event)" :aria-label="t('{name} target', { name: s.label })" />
          </td>
          <td class="mono mu">{{ s.power != null ? Math.round(s.power * 100) + '%' : s.speed != null ? t('fan {n}%', { n: Math.round(s.speed * 100) }) : '' }}</td>
        </tr>
      </tbody>
    </table></div>
  </section>
