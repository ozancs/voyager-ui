...
const pct = computed(() => (((local.value ?? 0) - props.min) / (props.max - props.min)) * 100 + '%')
...
<template>
  <div class="col" style="gap:6px">
    ...
    <input class="rng" type="range" :min="min" :max="max" :step="step" :value="local" :style="{ '--p': pct }" :aria-label="label" @input="onInput" @change="onChange" />
  </div>
</template>
