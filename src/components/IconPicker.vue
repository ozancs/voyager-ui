...
const more = computed(() => {
  ...
  return Object.entries(extra.value)
    .filter(([n, [, tg]]) => !words.value.length || hit(n.replace(/-/g, ' ') + ' ' + tg))
    .map(([n]) => 'l:' + n);
});
...
<template>
  <Modal :title="t('Choose icon')" width="680px" @close="emit('close')">
    <input
      ...
      v-model="q"
      class="input"
      :placeholder="t('Search icons, e.g. fan, heat, light, cut')"
    ...
    <div v-if="withColor" class="cr">
      ...
      <button
        ...
        @click="emit('color', c)"
      ></button>
      <label class="dot cu" :aria-label="t('Custom colour')"
        ><input type="color" :value="cur || '#ff6b1a'" @input="emit('color', $event.target.value)"
      /></label>
    </div>
    <div class="scr">
      <template v-if="basic.length">
        ...
        <div class="ig">
          <button
            ...
            @click="pick(n)"
          >
            <Icon :name="n" :size="22" :style="cur && icon !== n ? { color: cur } : null" />
          </button>
        </div>
      </template>
      <span class="lbl"
        >{{ t('More icons') }} <span class="mu">({{ more.length }})</span></span
      >
      ...
      <div class="ig">
        <button
          ...
          class="btn"
          :class="{ acc: icon === n }"
          :aria-label="n.slice(2)"
          :data-tip="n.slice(2)"
          @click="pick(n)"
        >
          <Icon :name="n" :size="22" :stroke="2" :style="cur && icon !== n ? { color: cur } : null" />
        </button>
      </div>
    </div>
  </Modal>
...
.dot.cu {
  position: relative;
  overflow: hidden;
  background: conic-gradient(#ff6b1a, #f5c451, #3dd68c, #38d6ff, #8b6cff, #ff3d7f, #ff6b1a);
}
.dot.cu input {
  position: absolute;
  inset: 0;
