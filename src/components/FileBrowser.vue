...
onMounted(async () => {
  try {
    roots.value = (await api.call('server.files.roots')).map((r) => r.name);
  } catch {}
  load();
});
let rt;
useApiEvent('notify_filelist_changed', () => {
  clearTimeout(rt);
...
function sortBy(k) {
  sort.value = { k, d: sort.value.k === k ? -sort.value.d : k === 'filename' ? 1 : -1 };
}
function changeRoot(r) {
  root.value = r;
  path.value = '';
...
function open(it) {
  if (it.dir) {
    path.value = path.value ? path.value + '/' + it.name : it.name;
    picked.value = new Set();
    load();
    return;
  }
  ...
  if (IMG.test(it.name)) {
    ...
    img.value = { list: rows.value.filter((x) => !x.dir && IMG.test(x.name)).map((x) => dir + x.name), start: rel };
    return;
  }
...
async function upload(e, sub = '') {
  ...
  for (const f of fs) {
    up.value = 0;
    try {
      await api.upload(f, { root: root.value, path: dest, onProgress: (p) => (up.value = p) });
      ok++;
    } catch (err) {
  ...
  up.value = null;
  if (ok) {
    toast(ok === 1 ? t('Uploaded {name}', { name: fs[0].name }) : t('Uploaded {n} files', { n: ok }));
    load();
  }
}
...
function togglePick(it) {
  ...
  s.has(key(it)) ? s.delete(key(it)) : s.add(key(it));
  picked.value = s;
}
...
function toggleAll() {
  picked.value = allPicked.value ? new Set() : new Set(rows.value.map(key));
}
function dlPicked() {
  const its = pickedRows.value;
  if (its.length === 1 && !its[0].dir) {
    const a = document.createElement('a');
    a.href = api.fileUrl(root.value, (path.value ? path.value + '/' : '') + its[0].name);
    a.download = its[0].name;
    a.click();
    return;
  }
  downloadMany(
    root.value,
    its.map((it) => ({ path: (path.value ? path.value + '/' : '') + it.name, dir: it.dir })),
    root.value,
  );
}
...
<template>
  <section
    ...
    @dragleave="dLeave"
    @dragover.prevent
    @drop.prevent="dDrop($event)"
  >
    <div v-if="over" class="dz">
      <div class="pill">
        <Icon name="upload" :size="20" :stroke="2.4" /><b>{{
          overDir ? t('Drop to upload into {dir}', { dir: overDir }) : t('Drop to upload to /{p}', { p: full })
        }}</b>
      </div>
    </div>
    <div class="card-h">
      <h2 class="row"><Icon name="folder" :size="18" />{{ t('Config Files') }}</h2>
    </div>
    <div class="row" style="flex-wrap: wrap">
      <select
        ...
        :value="root"
        :aria-label="t('Root')"
        @change="changeRoot($event.target.value)"
      >
      ...
      <button
        ...
        @click="modal = { kind: 'newfile', value: '' }"
      >
    ...
    <div v-if="picked.size" class="bulk row">
      ...
      <button class="btn dg" @click="modal = { kind: 'bulkdelete' }">
    ...
    <div class="row mono" style="justify-content: space-between; font-size: 12px">
      <b>{{ t('Current path: /{p}', { p: full }) }}</b
      ><span class="mu" v-if="disk">{{
        up != null
          ? t('Uploading {n}%', { n: Math.round(up * 100) })
          : t('Free disk: {free}', { free: fmtBytes(disk.free) })
      }}</span>
    </div>
      ...
      <table class="tbl">
        <thead>
          <tr>
            ...
            <th class="s" @click="sortBy('size')" style="text-align: right">{{ t('Filesize') }}</th>
            <th class="s" @click="sortBy('modified')" style="text-align: right">{{ t('Last modified') }}</th>
            <th style="width: 110px"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="path" class="click" @click="upDir">
            <td></td>
            <td><Icon name="folder" :size="18" style="color: var(--mu)" /></td>
            <td colspan="4" class="mono">..</td>
          </tr>
          <tr
            v-for="it in rows"
            :key="(it.dir ? 'd' : 'f') + it.name"
            class="click fr"
            :class="{ tgt: it.dir && overDir === it.name, sel: picked.has(key(it)) }"
            @click="open(it)"
            @dragover="it.dir && (overDir = it.name)"
            @dragleave="it.dir && overDir === it.name && (overDir = null)"
            @drop.prevent.stop="dDrop($event, it.dir ? it.name : '')"
          >
            <td @click.stop>
              <input
                type="checkbox"
                class="cb"
                :checked="picked.has(key(it))"
                :aria-label="t('Select {name}', { name: it.name })"
                @change="togglePick(it)"
              />
            </td>
            <td>
              <Icon
                :name="it.dir ? 'folder' : 'file'"
                :size="18"
                :style="{ color: it.dir ? 'var(--ac)' : 'var(--mu)' }"
              />
            </td>
            ...
            <td>
              <div class="acts">
                <a
                  ...
                  :href="api.fileUrl(root, (path ? path + '/' : '') + it.name)"
                  download
                ...
                <button
                  class="btn clear ibtn sm"
                  :aria-label="t('Rename')"
                  @click.stop="modal = { kind: 'rename', item: it, value: it.name }"
                >
                ...
                <button
                  class="btn clear ibtn sm"
                  :aria-label="t('Delete')"
                  @click.stop="modal = { kind: 'delete', item: it }"
                >
  ...
  <Modal v-if="modal" :title="t(TITLES[modal.kind])" @close="modal = null">
    <template v-if="modal.kind === 'bulkdelete'">
      <p style="margin: 0">
        {{ t('Delete {n} items? Folders are deleted with everything inside.', { n: pickedRows.length }) }}
      </p>
      <div class="mono mu" style="font-size: 12px; max-height: 160px; overflow: auto">
        {{ pickedRows.map((it) => it.name + (it.dir ? '/' : '')).join(', ') }}
      </div>
    </template>
    <!-- prettier-ignore -->
    ...
    <input
      ...
      v-model="modal.value"
      class="input mono"
      :aria-label="t(TITLES[modal.kind])"
      @keydown.enter="confirmModal"
    />
    <template #foot
      ><button class="btn lg" @click="modal = null">{{ t('Cancel') }}</button
      ><button
        class="btn lg"
        :class="modal.kind.endsWith('delete') ? 'dgf' : 'acc'"
        :disabled="!modal.kind.endsWith('delete') && !modal.value"
        @click="modal.kind === 'bulkdelete' ? deletePicked() : confirmModal()"
      >
        {{ modal.kind.endsWith('delete') ? t('Delete') : t('OK') }}
      </button></template
    >
  </Modal>
...
.bulk {
  ...
  border-radius: 12px;
  background: color-mix(in srgb, var(--ac) 12%, var(--s2));
  flex-wrap: wrap;
}
tr.sel td {
...
tr.tgt td {
  background: color-mix(in srgb, var(--ac) 18%, transparent) !important;
}
.nm {
  font-weight: 600;
