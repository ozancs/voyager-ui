...
async function load() {
  try {
    ...
    for (const f of files.value)
      api
        .call('server.files.metadata', { filename: f.path })
        .then((m) => (f.meta = m))
        .catch(() => {});
  } catch {}
}
...
useApiEvent('notify_filelist_changed', ([p]) => {
  if (p?.item?.root === 'gcodes') {
    clearTimeout(tm);
    tm = setTimeout(load, 500);
  }
});
function thumb(f) {
...
<template>
  <section class="card">
    <div class="card-h">
      ...
      <button class="btn clear" @click="go('files')">{{ t('All files') }}</button>
    </div>
    <div class="list">
      <div v-for="f in files" :key="f.path" class="fr">
        ...
        <div class="col grow" style="gap: 1px; min-width: 0">
          <b class="fn">{{ f.path.split('/').pop() }}</b
          ><span class="mono mu">{{ fmtTime(f.meta?.estimated_time) }} · {{ fmtDate(f.modified) }}</span>
        </div>
        <button
          v-if="state.queue.enabled"
          class="btn ibtn sm"
          :aria-label="t('Add to queue')"
          @click="queueApi.add([f.path])"
        >
          <Icon name="queue" :size="15" />
        </button>
        <button class="btn acc ibtn sm" :aria-label="t('Print')" :disabled="isPrinting" @click="print(f)">
          <Icon name="play" :size="15" :stroke="2.4" />
        </button>
      </div>
