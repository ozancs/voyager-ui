...
const pct = computed(() =>
  sp.value?.initial_weight
    ? Math.max(0, Math.min(100, (sp.value.remaining_weight / sp.value.initial_weight) * 100))
    : null,
);
...
const shown = computed(() =>
  spools.value.filter(
    (s) =>
      !q.value ||
      JSON.stringify([s.id, s.filament?.name, s.filament?.material, s.filament?.vendor?.name])
        .toLowerCase()
...
async function pick(s) {
  picking.value = false;
  try {
    await api.call('server.spoolman.post_spool_id', { spool_id: s ? s.id : null });
    await loadSpool();
  } catch (e) {
    toast(e.message, 'error');
  }
...
<template>
  <section v-bind="$attrs" class="card t-spool">
    <div class="card-h">
      <h2>{{ t('Spool') }}</h2>
      <button class="btn" @click="openPick"><Icon name="spool" :size="16" />{{ t('Change') }}</button>
    </div>
    <div v-if="sp" class="row" style="gap: 16px">
      ...
      <div class="col grow" style="gap: 4px; min-width: 0">
        <b style="font-size: 16px"
          >{{ sp.filament?.name || t('Spool') }} <span class="mu">#{{ sp.id }}</span></b
        >
        ...
        <span class="mono"
          >{{ Math.round(sp.remaining_weight || 0) }} g
          <span class="mu">/ {{ Math.round(sp.initial_weight || 0) }} g</span></span
        >
      </div>
    </div>
    <div v-else class="empty">
      {{ state.spoolman.server ? t('No active spool') : t('Spoolman is not set up in moonraker.conf') }}
    </div>
  </section>
  <Modal v-if="picking" :title="t('Choose spool')" width="560px" @close="picking = false">
    ...
    <div class="pl">
      <button v-for="s in shown" :key="s.id" class="btn clear pi" :class="{ on: sp?.id === s.id }" @click="pick(s)">
        ...
        <span class="grow" style="text-align: left"
          >#{{ s.id }} {{ s.filament?.name }} <span class="mu">{{ s.filament?.material }}</span></span
        >
    ...
    <template #foot
      ><button class="btn lg" @click="pick(null)">{{ t('Clear active spool') }}</button></template
    >
  </Modal>
</template>
