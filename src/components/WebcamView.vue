...
const errMsg = ref('')
let timer = null, frames = 0, fpsTimer = null, busy = false, watchdog = null, pc = null, hls = null, gen = 0
...
function gotFrame() { frames++; if (status.value !== 'live') { status.value = 'live'; clearTimeout(watchdog) } }
function fail(msg) { status.value = 'error'; errMsg.value = msg || '' }
function poll() {
  if (busy) return
  busy = true
  const img = new Image()
  img.onload = () => { src.value = img.src; gotFrame(); busy = false }
  img.onerror = () => { busy = false }
  img.src = snapUrl()
}
const post = (url, body, type = 'application/json') => fetch(url, { method: 'POST', body, headers: { 'Content-Type': type } }).then((r) => { if (!r.ok) throw new Error('HTTP ' + r.status); return r })

async function startCameraStreamer(g) {
  const url = streamUrl.value
  const offer = await (await post(url, JSON.stringify({ type: 'request', iceServers: [{ urls: ['stun:stun.l.google.com:19302'] }], keepAlive: true }))).json()
  if (g !== gen) return
  pc = new RTCPeerConnection({ iceServers: offer.iceServers || [] })
  pc.addTransceiver('video', { direction: 'recvonly' })
  pc.ontrack = (e) => { if (video.value) video.value.srcObject = e.streams[0] }
  pc.onicecandidate = (e) => { if (e.candidate) post(url, JSON.stringify({ id: offer.id, type: 'remote_candidate', candidates: [e.candidate] })).catch(() => {}) }
...
async function startSdpPost(url, g) {
  ...
  pc.ontrack = (e) => { if (video.value && e.track.kind === 'video') video.value.srcObject = e.streams[0] }
  const offer = await pc.createOffer()
  await pc.setLocalDescription(offer)
  // wait briefly for ICE gathering so the offer carries candidates (no trickle over plain HTTP)
  await new Promise((r) => { if (pc.iceGatheringState === 'complete') return r(); const to = setTimeout(r, 1500); pc.onicegatheringstatechange = () => { if (pc.iceGatheringState === 'complete') { clearTimeout(to); r() } } })
  if (g !== gen) return
...
async function startHls() {
  const v = video.value
  if (v.canPlayType('application/vnd.apple.mpegurl')) { v.src = streamUrl.value; return }
  const { default: Hls } = await import('hls.js')
  if (!Hls.isSupported()) throw new Error('HLS not supported by this browser')
  hls = new Hls({ lowLatencyMode: true })
  hls.on(Hls.Events.ERROR, (_, d) => { if (d.fatal) fail(d.details) })
...
async function start() {
  ...
  errMsg.value = ''
  if (mode.value === 'unsupported' || mode.value === 'iframe') { if (mode.value === 'iframe') status.value = 'live'; return }
  watchdog = setTimeout(() => { if (status.value === 'loading') fail() }, 10000)
  ...
  try {
    if (mode.value === 'mjpeg') {
      ...
      timer = setInterval(() => { if (imgEl.value?.naturalWidth > 0) { gotFrame(); clearInterval(timer) } }, 400)
    }
    else if (mode.value === 'snapshot') {
      const iv = 1000 / Math.max(1, Math.min(props.cam.target_fps || 10, 15))
      timer = setInterval(poll, iv); poll()
    } else {
      await nextTick()
  ...
  } catch (e) { if (g === gen) fail(e.message) }
}
function stop() {
  clearInterval(timer); clearInterval(fpsTimer); clearTimeout(watchdog)
  src.value = ''
  try { pc?.close() } catch {}
  pc = null
  try { hls?.destroy() } catch {}
  hls = null
  if (video.value) { video.value.srcObject = null; video.value.removeAttribute('src') }
}
function onVis() { document.hidden ? stop() : start() }
onMounted(() => { start(); document.addEventListener('visibilitychange', onVis) })
onBeforeUnmount(() => { gen++; stop(); document.removeEventListener('visibilitychange', onVis) })
watch(() => props.cam, start)
defineExpose({ retry: start })
</script>
<template>
  <div class="wc">
    <img v-if="mode === 'mjpeg' || mode === 'snapshot'" ref="imgEl" v-show="src && status !== 'error'" :src="src" :style="{ transform }" :alt="t('Webcam')" @load="gotFrame" @error="mode === 'mjpeg' && fail()" />
    <video v-else-if="isVideo" ref="video" v-show="status !== 'error'" autoplay muted playsinline :style="{ transform }" @playing="gotFrame" @timeupdate="gotFrame"></video>
    <iframe v-else-if="mode === 'iframe'" :src="streamUrl" :title="cam?.name || t('Webcam')" allow="autoplay; fullscreen"></iframe>

    <div v-if="!cam" class="msg"><Icon name="cam" :size="28" /><b>{{ t('No webcam configured') }}</b><span>{{ t('Add one in Mainsail or in moonraker.conf with a [webcam] section.') }}</span></div>
    <div v-else-if="status === 'loading'" class="msg"><Icon name="refresh" :size="22" class="spin" /><span>{{ t('Connecting to {name}…', { name: cam.name }) }}</span></div>
    <div v-else-if="status === 'error'" class="msg">
      <Icon name="warn" :size="26" />
      <b>{{ t('No picture from {name}', { name: cam.name }) }}</b>
      <span>{{ t('Check that the camera service (crowsnest) is running and the stream address opens in the browser.') }}</span>
    ...
    <div v-else-if="status === 'unsupported'" class="msg">
      ...
      <span>{{ t('Switch this camera to MJPEG or WebRTC (camera-streamer, go2rtc, MediaMTX) in the webcam settings.') }}</span>
    </div>
...
.wc img, .wc video, .wc iframe { width: 100%; height: 100%; object-fit: contain; position: absolute; inset: 0; border: 0; }
.msg { position: relative; z-index: 1; display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 20px; max-width: 420px; text-align: center; color: #b8bcc4; font-size: 13px; }
.msg b { color: #f0f0f0; font-size: 14px; }
.msg code { font-family: var(--fm); font-size: 11.5px; color: #9aa0a8; background: rgba(255,255,255,.06); padding: 3px 8px; border-radius: 6px; word-break: break-all; }
