...
for (const [k, [name, loader]] of Object.entries(LOADERS)) {
  VIEWS[k] = defineAsyncComponent({
    loader,
    delay: 0,
    loadingComponent: {
      render: () => h(LoadingPanel, { title: t('Opening {name}…', { name: t(name) }), compact: true }),
    },
  });
}
...
watch(
  ...
  (n) => {
    if (n === 'theme') {
      state.settingsOpen = state.settingsOpen || 'general';
      go('dashboard');
    }
  },
  { immediate: true },
...
watch(
  () => state.settingsLoaded && state.connected,
  (ok) => {
    if (ok) initSync();
  },
  { immediate: true },
...
function pinNav() {
  state.settings.navMode = navMode.value === 'pinned' ? 'hidden' : 'pinned';
  navOpen.value = false;
}
function toggleNav() {
...
function peek(on) {
  clearTimeout(navT);
  if (on) navOpen.value = true;
  else navT = setTimeout(() => (navOpen.value = false), 350);
}
const location = window.location;
const bootTask = computed(() => activeTasks.value[0]?.label || t('Loading printer'));
const showFavBar = computed(() => {
  ...
  <div class="shell" :class="{ booting: state.connected && !state.booted }">
    ...
    <div class="body">
      <SideNav
        ...
        @close="navOpen = false"
        @pin="pinNav"
        @mouseenter="navMode === 'auto' && peek(true)"
        @mouseleave="navMode === 'auto' && peek(false)"
      />
      <div v-if="navMode === 'auto' && !navOpen" class="edge" @mouseenter="peek(true)"></div>
      <button
        v-if="navMode !== 'pinned' && !navOpen && !narrow"
        class="navtab"
        :aria-label="t('Show / hide the side menu')"
        :data-tip="t('Menu')"
        @click="navMode === 'hidden' ? pinNav() : (navOpen = true)"
        @mouseenter="navMode === 'auto' && peek(true)"
      >
        <Icon name="chevr2" :size="16" :stroke="2.4" />
      </button>
      <main class="main">
        <div v-if="!state.connected" class="banner">
          <Icon name="refresh" :size="20" class="spin" />
          <div class="grow">
            <b>{{ t('Connecting to Moonraker…') }}</b
            ><span
              v-if="state.conn.attempts"
              class="mono"
              style="font-weight: 400; font-size: 12px; margin-left: 10px; color: var(--mu)"
              >{{ t('attempt {n}', { n: state.conn.attempts }) }}</span
            >
        ...
        <component
          :is="view"
          :key="route.name === 'config' ? 'config' : route.name"
        ...
        <footer class="ft mono">
          ...
          <Handoff />
          <a :href="'http://' + location.hostname + '/'" target="_blank" rel="noopener">{{
            t('Open {name}', { name: 'Mainsail' })
          }}</a>
        </footer>
      </main>
    </div>
