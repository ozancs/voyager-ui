...
const STYLE_OK = /^(\s*(color|background|background-color|font-weight|font-style|text-decoration)\s*:\s*[#\w\s(),.%-]+;?)+\s*$/i
export function richHtml(text) {
  const s = esc(String(text ?? ''))
  if (!s.includes('&lt;') && !s.includes('&amp;')) return s
  return s
    .replace(/&lt;span\s+style=&quot;([^&<>]*)&quot;\s*&gt;/gi, (m, st) => (STYLE_OK.test(st) && !/url|expression/i.test(st) ? `<span style="${st}">` : m))
