...
export class Moonraker {
  ...
  headers(extra = {}) {
    return this.auth.token ? { ...extra, Authorization: 'Bearer ' + this.auth.token } : extra;
  }
  ...
  async fetch(path, opts = {}, retry = true) {
    const r = await fetch(this.httpBase + (path.startsWith('/') ? path : '/' + path), {
      cache: 'no-store',
      ...opts,
      headers: this.headers(opts.headers),
    });
    if (r.status === 401 && retry && this.auth.refresh && (await this.refreshToken()))
      return this.fetch(path, opts, false);
    return r;
  }
  async refreshToken() {
    try {
      const r = await fetch(this.httpBase + '/access/refresh_jwt', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ refresh_token: this.auth.refresh }),
      });
  ...
  async login(username, password, source = 'moonraker') {
    const r = await fetch(this.httpBase + '/access/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async logout() {
    try {
      await this.fetch('/access/logout', { method: 'POST' }, false);
    } catch {}
    this.auth = { token: '', refresh: '', user: '' };
    this.saveAuth();
    try {
  ...
  async wsQuery() {
    let r;
    try {
      r = await this.fetch('/server/info');
    } catch {
      return '';
    } // network error: let the websocket fail and retry
    if (r.status === 401 || r.status === 403) {
      this.emit('auth-required', await this.authInfo());
      return null;
    }
  ...
  async authInfo() {
    try {
      const r = await fetch(this.httpBase + '/access/info');
  ...
  call(method, params) {
    ...
    if (done) {
      const tm = setTimeout(done, 30000);
      const fin = () => {
        clearTimeout(tm);
        done();
      };
