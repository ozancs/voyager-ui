...
function targets() {
  ...
  for (const h of heaters) {
    ...
    if (h === 'extruder')
      al.push('nozzle', 'hotend', 'hot end', 'ext', 'nozul', 'nozzle temp', 'ekstruder', 'uç', 'tool', 't0');
    else if (/^extruder\d+$/.test(h)) {
      const n = h.slice(8);
      al.push('e' + n, 'tool' + n, 't' + n);
    } else if (h === 'heater_bed')
      al.push('bed', 'heater bed', 'heatbed', 'hotbed', 'plate', 'tabla', 'yatak', 'bed temp');
  ...
  for (const o of state.objects) {
    ...
    else if (type === 'fan_generic')
      out.push({
        kind: 'fan',
        obj: o,
        al: [norm(shortName(o)), norm(shortName(o)) + ' fan', norm(prettyName(o))],
        pri: 3,
      });
    else if (type === 'output_pin' && !o.startsWith('output_pin _'))
      out.push({ kind: 'pin', obj: o, al: [norm(shortName(o)), norm(prettyName(o))], pri: 2 });
    else if (['neopixel', 'led', 'dotstar', 'pca9533', 'pca9632'].includes(type))
      out.push({
        kind: 'led',
        obj: o,
        al: [norm(shortName(o)), norm(prettyName(o)), 'led', 'leds', 'light', 'lights', 'ışık'],
        pri: 2,
      });
  }
  out.push(
  ...
  for (const a of ['x', 'y', 'z'])
    out.push({ kind: 'move', axis: a, al: [a, 'move ' + a, a + ' axis', 'go ' + a], pri: 1 });
  return out;
}
...
function parse(raw) {
  ...
  // "<number><unit> <name>"
  if ((m = q.match(/^([-+]?\d*\.?\d+)\s*(%|°c|°|c)?\s+(.+)$/)))
    return { name: m[3].trim(), num: parseFloat(m[1]), signed: /^[-+]/.test(m[1]), unit: m[2] || '' };
  // "<name> off|on|max|<color>"
  const words = q.split(' ');
  const last = words[words.length - 1];
  if (words.length >= 2 && (OFF.includes(last) || ON.includes(last) || COLORS[last]))
    return { name: words.slice(0, -1).join(' '), word: last };
  return { name: q };
}
...
export function smartResults(raw) {
  ...
  if (p.num == null && !p.word) {
    for (const pr of state.settings.presets || []) {
      ...
      if (name === pn || name === 'preheat ' + pn || name === pn + ' preheat' || name === pn + ' ısıt') {
        const txt = Object.entries(pr.temps || {})
          .filter(([h, v]) => v && state.objects.includes(h))
          .map(([h, v]) => `${prettyName(h)} ${v}°`)
          .join(', ');
        res({
          t: t('Preheat {name}', { name: pr.name }),
          s: txt,
          icon: 'flame',
          run: () => applyPreset(pr),
          sc: 990,
    ...
    if ((m = name.match(/^(home|g28|homing)(?: (all|[xyz]{1,3}|[xyz](?: [xyz]){0,2}))?$/))) {
      ...
      res({
        t: ax ? t('Home {axes}', { axes: ax.split('').join(', ') }) : t('Home all axes'),
        s: 'G28' + (ax ? ' ' + ax.split('').join(' ') : ''),
        icon: 'home',
        run: () => gcode('G28' + (ax ? ' ' + ax.split('').join(' ') : '')),
        sc: 990,
        confirm: true,
      });
    }
  ...
  const cands = targets()
    .map((tg) => ({ tg, sc: nameScore(name, tg.al) }))
    .filter((c) => c.sc >= 40);
...
function build(tg, p, homed) {
  const n = p.num,
    off = OFF.includes(p.word),
    on = ON.includes(p.word);
  ...
  switch (tg.kind) {
    ...
    case 'tempfan': {
      ...
      const s = S(tg.obj),
        max = cfg(tg.obj).max_temp,
        min = cfg(tg.obj).min_temp;
      const cmd =
        tg.kind === 'tempfan'
          ? `SET_TEMPERATURE_FAN_TARGET TEMPERATURE_FAN=${shortName(tg.obj)} TARGET=${v}`
          : `SET_HEATER_TEMPERATURE HEATER=${shortName(tg.obj)} TARGET=${v}`;
      const bad =
        max != null && v > max
          ? t('Above max_temp ({max}°)', { max })
          : v > 0 && min != null && v < min
            ? t('Below min_temp ({min}°)', { min })
            : '';
      return {
        t: v === 0 ? t('Turn off {name}', { name: nm }) : t('Set {name} to {v}°', { name: nm, v: fmt(v) }),
        s: `${t('now')} ${fmt(s.temperature)}° → ${v}°  ·  ${cmd}`,
        icon: 'flame',
        run: () => setHeater(tg.obj, v),
        warn: bad,
        disabled: !!bad,
        confirm: v > 0,
      };
    ...
    case 'fan': {
      ...
      if (v == null || v < 0 || v > 100)
        return {
          t: t('Set {name} to {v}%', { name: nm, v: fmt(v, 0) }),
          s: '',
          icon: 'fan',
          warn: t('Fan speed is 0-100%'),
          disabled: true,
          run: () => {},
        };
      return {
        t: v === 0 ? t('Turn off {name}', { name: nm }) : t('Set {name} to {v}%', { name: nm, v: fmt(v, 0) }),
        s: `${t('now')} ${Math.round((S(tg.obj).speed || 0) * 100)}% → ${fmt(v, 0)}%`,
        icon: 'fan',
        run: () => setFan(tg.obj, v),
      };
    }
    case 'pin': {
      ...
      return {
        t: t('Set {name} to {v}', { name: nm, v: shown }),
        s: `SET_PIN PIN=${shortName(tg.obj)} VALUE=${+v.toFixed(3)}`,
        icon: 'bulb',
        run: () => gcode(`SET_PIN PIN=${shortName(tg.obj)} VALUE=${+v.toFixed(3)}`),
      };
    }
    case 'led': {
      let hex = off ? '#000000' : on ? '#ffffff' : COLORS[p.word];
      if (!hex && n != null) {
        const g = Math.max(0, Math.min(100, n)) / 100;
        const h = Math.round(g * 255)
          .toString(16)
          .padStart(2, '0');
        hex = '#' + h + h + h;
      }
      ...
      return {
        t: off
          ? t('Turn off {name}', { name: nm })
          : t('Set {name} to {v}', { name: nm, v: p.word || (n != null ? n + '%' : hex) }),
        s: cmd,
        icon: 'bulb',
        swatch: hex,
        run: () => gcode(cmd),
    ...
    case 'flow': {
      ...
      const ok = n >= 1 && n <= 500;
      return {
        t: t(tg.kind === 'speed' ? 'Set speed factor to {v}%' : 'Set flow to {v}%', { v: fmt(n, 0) }),
        s: `${t('now')} ${Math.round((cur || 1) * 100)}%  ·  ${tg.kind === 'speed' ? 'M220' : 'M221'} S${Math.round(n)}`,
        icon: tg.kind === 'speed' ? 'bolt' : 'drop',
        run: () => gcode(`${tg.kind === 'speed' ? 'M220' : 'M221'} S${Math.round(n)}`),
        disabled: !ok,
        warn: ok ? '' : '1-500%',
      };
    }
    case 'pa': {
      ...
      return {
        t: t('Set pressure advance to {v}', { v: n }),
        s: `${t('now')} ${fmt(S('extruder').pressure_advance, 4)}  ·  SET_PRESSURE_ADVANCE ADVANCE=${n}`,
        icon: 'load',
        run: () => gcode(`SET_PRESSURE_ADVANCE ADVANCE=${n}`),
      };
    }
    case 'zoff': {
      ...
      return {
        t: adjust
          ? t('Adjust Z offset by {v} mm', { v: (n > 0 ? '+' : '') + n })
          : t('Set Z offset to {v} mm', { v: n }),
        s: `${t('now')} ${fmt(cur, 3)} → ${fmt(adjust ? cur + n : n, 3)}  ·  ${cmd}`,
        icon: 'target',
        run: () => gcode(cmd),
        warn: homed.includes('z') ? '' : t('Home Z first'),
        disabled: !homed.includes('z'),
        confirm: true,
      };
    }
    case 'accel':
    ...
    case 'mcr': {
      ...
      const K = {
        accel: ['ACCEL', 'max_accel', 'mm/s²', 'Set max acceleration to {v}'],
        velocity: ['VELOCITY', 'max_velocity', 'mm/s', 'Set max velocity to {v}'],
        scv: ['SQUARE_CORNER_VELOCITY', 'square_corner_velocity', 'mm/s', 'Set square corner velocity to {v}'],
        mcr: ['MINIMUM_CRUISE_RATIO', 'minimum_cruise_ratio', '', 'Set minimum cruise ratio to {v}'],
      }[tg.kind];
      if (tg.kind === 'mcr' && n >= 1) return null;
      const cur = S('toolhead')[K[1]];
      return {
        t: t(K[3], { v: n + (K[2] ? ' ' + K[2] : '') }),
        s: `${t('now')} ${fmt(cur, 2)}  ·  SET_VELOCITY_LIMIT ${K[0]}=${n}`,
        icon: 'bolt',
        run: () => gcode(`SET_VELOCITY_LIMIT ${K[0]}=${n}`),
      };
    }
    case 'extrude':
    case 'retract': {
    ...
    case 'move': {
      if (n == null || p.word) return null;
      const i = 'xyz'.indexOf(tg.axis),
        A = tg.axis.toUpperCase();
      const pos = S('toolhead').position?.[i],
        min = S('toolhead').axis_minimum?.[i],
        max = S('toolhead').axis_maximum?.[i];
      const rel = p.signed;
...
export function rawCommand(raw) {
  ...
  const known =
    Object.keys(state.commands).some((k) => k.toUpperCase() === first) ||
    state.objects.includes('gcode_macro ' + first) ||
    /^[GM]\d+$/.test(first);
  if (!known || (!/\s/.test(q) && !/^[GM]\d+$/.test(first))) return null;
  const cmd = first + q.slice(first.length);
  return {
    t: t('Run {cmd}', { cmd }),
    s: t('Sends this line as typed'),
    icon: 'term',
    cat: t('Quick action'),
    dest: t('Runs command'),
    run: () => gcode(cmd),
